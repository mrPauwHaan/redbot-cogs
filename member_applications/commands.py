...
class JoinRequestView(discord.ui.View):
    ...
    async def reject_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        # Als er nog geen 3 stemmen zijn, update de knop op het bericht
        if votes < 3:
...
class memberapplications(commands.Cog):
    ...
    async def post_to_intro_forum(self, guild: discord.Guild, user_id: int, original_embed: discord.Embed):
        ...
        try:
            ...
            intro_embed.set_footer(
                text="Shadowzone Welkomsteam",
                icon_url=guild.icon.url if guild.icon else None
            )

            await channel.create_thread(
                name=thread_title,
                content=f"Welkom in Shadowzone <@{user_id}>! 🎉 Stel je gerust verder voor of klets gezellig mee.",
                embed=intro_embed
            )
    ...
    async def _process_single_request(self, req: dict, guild: discord.Guild) -> bool:
        """Verwerkt één en enkel openstaand (SUBMITTED) verzoek via een gestylede Embed + View."""
        try:
            ...
            # Haal member op voor profielfoto
            member = guild.get_member(user_id)
            if not member:
                try:
                    member = await guild.fetch_member(user_id)
            ...
            # Bouw de strakke Embed kaart op
            embed = discord.Embed(
